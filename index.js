...
function createMenuItem(name, cost, category) {
  const tea = { name, cost, category };
  return tea;
  const dessert = { name, cost, category };
  return dessert;
  const snack = { name, cost, category };
  return snack;
}

/* Task 1b: use your function to create 3 more menu items. You may add any items to the menu that you'd like */
createMenuItem("Earl Gray", 4, "Drinks");
createMenuItem("Carrot Cake", 4.5, "Dessert");
createMenuItem("Hummus", 3, "Snacks");
...
reviews.push({
  name: "sarah",
  rating: 5,
  feedback: "wonderful server! always great variety of food ",
});
console.log(reviews);
...
function getReviewByIndex(array, index) {
  return `${array[index].name}  gave the restaraunt a ${array[index].rating} and their feedback was: ${array[index].feedback}`;
}
